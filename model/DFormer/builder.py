...
class EncoderDecoder(nn.Module):
    def __init__(self, cfg=None, criterion=nn.CrossEntropyLoss(reduction='mean', ignore_index=255), norm_layer=nn.BatchNorm2d, single_GPU=True):
        ...
        if cfg.drop_path_rate is not None:
            self.backbone = backbone(drop_path_rate=cfg.drop_path_rate, norm_cfg = norm_cfg, x_channels=cfg.x_channels, x_e_channels=cfg.x_e_channels)
        else:
            self.backbone = backbone(drop_path_rate=0.1, norm_cfg = norm_cfg, x_channels=cfg.x_channels, x_e_channels=cfg.x_e_channels)
