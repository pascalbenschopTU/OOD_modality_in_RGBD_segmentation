...
class DFormer(BaseModule):
    def __init__(self, in_channels=4, depths=(2, 2, 8, 2), dims=(32, 64, 128, 256), out_indices=(0, 1, 2, 3), windows=[7, 7, 7, 7], norm_cfg=dict(type='SyncBN', requires_grad=True),
        ...
        self.x_e_channels = x_e_channels
        self.downsample_layers = nn.ModuleList() 
        stem = nn.Sequential(
                nn.Conv2d(x_channels, dims[0] // 2, kernel_size=3, stride=2, padding=1),
        ...
        stem_e = nn.Sequential(
                nn.Conv2d(x_e_channels, dims[0] // 4, kernel_size=3, stride=2, padding=1),
    ...
    def init_weights(self,pretrained):
        ...
        for k, v in _state_dict.items():
            if k.startswith('backbone.'):
                state_dict[k[9:]] = v
            elif k.startswith('downsample_layers.0.0.weight') and self.x_channels == 1:
                state_dict[k] = v[:, :1, :, :]
            elif k.startswith('downsample_layers.0.0.weight') and self.x_channels == 3:
                state_dict[k] = v.repeat(1, 3, 1, 1)
            else:
                state_dict[k] = v
    ...
    def forward(self, x,x_e):
        ...
        if self.x_channels == 1:
            x = x_e
        if self.x_e_channels == 3:
            x_e = x
